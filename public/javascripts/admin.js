...
// const hostURL= `kkms4001.iptime.org`;
...
const fetchJsonFile = async () => {
  ...
  try {
    const response = await fetch(`${hostURL}:${portJ}/energyData`);
    if (!response.ok) {
...
const parseData = (jData, rangeN) => {
  ...
  dayDataList.forEach((v, i) => {
    ...
    const formattedMonth = calcMonth <= 9 ? `0${calcMonth}` : `${calcMonth}`;
    const formattedDay = calcDay - totalDays <= 9 ? `0${calcDay - totalDays}` : `${calcDay - totalDays}`;
    dayList.push(`2024-${formattedMonth}-${formattedDay}`);
...
const urlMove = () => {
  document.getElementById("home_move").addEventListener("click", () => {
    const changedURL = `${hostURL}:${portN}/home`;
